...
sim.createInitialObjEvt = function () {
  if (sim.setupInitialState) sim.setupInitialState();
}
/*******************************************************************
...
sim.initializeScenarioRun = function (expParamSlots, seed) {
  ...
  */
  ...
  if (sim.setupStatistics) sim.setupStatistics();
};
/*******************************************************
...
sim.runScenario = function () {
  ...
  if (sim.computeFinalStatistics) sim.computeFinalStatistics();
}
/*******************************************************
  ...
  for (let i=0; i < M; i++) {
    ...
    if (N > 0) {  // parameter variation experiment
      ...
      self.postMessage({
        ...
        expScenStat: exp.scenarios[i].stat  // aggregate statistics
      });
    }
  }
